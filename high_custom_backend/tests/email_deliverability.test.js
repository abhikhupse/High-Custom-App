...
test("default sequence format sends the complete saved sequence", async () => {
  ...
  try {
    const options = {
      sequence: {
        ...
        brand: { logoUrl: "https://example.com/logo" },
        heroImage: {
          url: "https://example.com/hero.jpg",
          link: "https://example.com/landing",
        },
        actionLinks: {
          cta: { enabled: true, text: "Buy", url: "https://example.com/buy" },
        },
        attachment: { name: "Brochure", url: "/brochure.pdf" },
      },
      lead: { firstName: "Ana" },
      baseUrl: "https://example.com",
      trackingUrl: "https://example.com/pixel",
      interestedUrl: "https://example.com/interested",
    ...
    for (const provider of ["gmail", "zoho"]) {
      ...
      assert.match(bodies.html, /example.com\/logo/);
      assert.match(bodies.html, /example.com\/hero.jpg/);
      assert.match(bodies.html, /example.com\/landing/);
      assert.match(bodies.html, /example.com\/buy/);
      assert.match(bodies.html, /brochure.pdf/);
      assert.doesNotMatch(bodies.html, /example.com\/pixel/);
      assert.match(bodies.text, /Hi Ana,\nCan we talk?/);
      assert.match(
        bodies.text,
      ...
      assert.match(mime, /example.com\/buy/);
      assert.match(mime, /brochure.pdf/);
      assert.match(mime, /example.com\/hero.jpg/);
      assert.doesNotMatch(mime, /example.com\/pixel/);
    }
...
test("personal HTML includes a unique open pixel when sequence tracking is enabled", () => {
  ...
  try {
    const enabled = buildSequenceBodies({
      sequence: { content: "Hello", tracking: { enabled: true } },
      lead: {},
      trackingUrl: "https://example.com/open/unique?v=123",
    });
    assert.match(
      enabled.html,
      /src="https:\/\/example.com\/open\/unique\?v=123"/,
    );
    const disabled = buildSequenceBodies({
      sequence: { content: "Hello", tracking: { enabled: false } },
      lead: {},
      trackingUrl: "https://example.com/open/unique?v=123",
    });
...
test("DSN parser suppresses invalid recipients but not policy blocks, full mailboxes, or text claims", () => {
  const part = (status, action = "failed") => ({
  ...
  assert.deepEqual(permanentFailures({ parts: [part("5.1.1")] }), [
    "lead@example.com",
  ]);
  for (const status of ["4.1.1", "5.7.1", "5.2.2"])
    assert.deepEqual(permanentFailures(part(status)), []);
  assert.deepEqual(permanentFailures(part("5.1.1", "delayed")), []);
  assert.deepEqual(
    permanentFailures({ ...part("5.1.1"), mimeType: "text/plain" }),
    [],
  );
});
...
test("placeholder names never become a greeting", () => {
  for (const firstName of ["-", " — ", "N/A", "unknown", "", "  "]) {
    assert.equal(
      replaceLeadPlaceholders("Hi {{firstName}},", { firstName }),
      "Hi there,",
    );
  }
  assert.equal(
    replaceLeadPlaceholders("Hi {{fullName}},", {
      firstName: "Ana",
      lastName: "-",
    }),
    "Hi Ana,",
...
test("rich editor formatting is preserved while unsafe HTML stays inert", () => {
  const html = buildSequenceEmail({
    sequence: {
      content:
        'Hello <strong>{{firstName}}</strong> <a href="https://example.com">visit</a><script>alert(1)</script>',
    },
    lead: { firstName: "Ana" },
  });
...
test("inline logo and banner sources are preserved in the email template", () => {
  const html = buildSequenceEmail({
    sequence: {
      brand: { logoUrl: "cid:sequence-brand-logo" },
      heroImage: { url: "cid:sequence-hero-image" },
      content: "Hello",
    },
  });
  assert.match(html, /src="cid:sequence-brand-logo"/);
  assert.match(html, /src="cid:sequence-hero-image"/);
});
...
test("MIME message retains real file attachments", async () => {
  const raw = await createMimeMessage({
    from: "sender@example.com",
    to: "lead@example.com",
    subject: "Document",
    ...
    attachments: [
      {
        ...
        content: "High Custom brochure",
        contentDisposition: "attachment",
      },
    ],
  });
  const mime = Buffer.from(raw, "base64url").toString();
  assert.match(mime, /filename=brochure\.txt/);
...
test("tracking-free HTML still includes unsubscribe and text preserves document links", () => {
  ...
  try {
    const options = {
      sequence: {
        content: "Hi {{firstName}},",
        attachment: { url: "/file.pdf", name: "Brochure" },
      },
      lead: { firstName: "-" },
      baseUrl: "https://example.com",
      trackingUrl: "https://example.com/pixel",
      interestedUrl: "https://example.com/yes",
      notInterestedUrl: "https://example.com/unsubscribe",
    };
    ...
    for (const [key, value] of [
      ["EMAIL_OPEN_TRACKING_ENABLED", oldPixel],
      ["EMAIL_RESPONSE_BUTTONS_ENABLED", oldButtons],
    ]) {
