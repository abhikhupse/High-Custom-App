(function () {
    ...
    if (!document.getElementById('adminMobileResponsiveCss')) {
        ...
        responsiveCss.href = '/css/admin-mobile-responsive.css';
    ...
    const organiseAdminFlow = () => {
        ...
        [social, link, sequence, tracking].filter(Boolean).forEach((item) => submenu.append(item.closest('.nav-item')));
        if (leads) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        prepareMobileTableCards();
        new MutationObserver(prepareMobileTableCards).observe(document.body, { childList: true, subtree: true });
    });
    ...
    document.querySelectorAll('.sidebar .nav-link').forEach((link) => {
        ...
        link.setAttribute('href', route[1]);

        if (currentPath.toLowerCase() === route[1].toLowerCase()) {
