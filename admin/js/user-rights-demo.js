document.addEventListener("DOMContentLoaded", function () {
  ...
  rows.forEach(function (row, rowIndex) {
    ...
    if (appSelect) {
      const selected =
        appSelect.value === "All Applications"
          ? [
              "Dashboard",
              "Leads",
              "Social Links",
    ...
    if (statusBadge) {
      ...
      wrapper.className =
        "form-check form-switch d-flex justify-content-center m-0";
      wrapper.innerHTML =
        '<input class="form-check-input user-status-switch" type="checkbox" aria-label="Toggle status for ' +
        name +
        '" ' +
        (statusBadge.textContent.trim() === "Active" ? "checked" : "") +
        ">";
  ...
  function makeEditButton(type, name, value) {
    ...
    button.setAttribute(
      "aria-label",
      "Edit " + (type === "app" ? "app" : "access") + " rights for " + name,
    );
  ...
  function openRightsModal(event) {
    ...
    document.getElementById("rightsModalTitle").textContent = isApp
      ? "Select App Rights"
      : "Select Access Right";
    ...
    if (isApp) {
      ...
      document
        .querySelectorAll(".app-right-check")
  ...
  document
    .getElementById("saveRightsBtn")
    .addEventListener("click", function () {
      if (!activeButton) return;

      if (activeButton.dataset.rightType === "app") {
        const selected = Array.from(
          document.querySelectorAll(".app-right-check:checked"),
        ).map(function (checkbox) {
          return checkbox.value;
        });
        activeButton.dataset.value = selected.join(",");
        activeButton.title =
          "App Rights: " + (selected.length ? selected.join(", ") : "None");
      } else {
