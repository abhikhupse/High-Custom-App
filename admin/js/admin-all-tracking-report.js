(function () {
  ...
  function pagination(meta) {
    ...
    const pages = Math.max(Number(meta.totalPages || 1), 1);
  ...
  async function render() {
    const search =
      document.getElementById("adminTrackingSearch")?.value.trim() || "";
    const userId = document.getElementById("adminTrackingUser")?.value || "";
    const businessType =
      document.getElementById("adminTrackingBusinessType")?.value || "";
    const step = document.getElementById("adminTrackingStep")?.value || "";
    const statusValue =
      document.getElementById("adminTrackingStatus")?.value || "";
    ...
    Object.entries(
      range(document.getElementById("adminTrackingDateRange")?.value || ""),
    ...
    try {
      ...
      const businessSelect = document.getElementById(
        "adminTrackingBusinessType",
      );
      if (businessSelect && !businessSelect.dataset.loaded) {
        ...
        businessSelect.dataset.loaded = "true";
      }
      const stepSelect = document.getElementById("adminTrackingStep");
      if (stepSelect && !stepSelect.dataset.loaded) {
        stepSelect.insertAdjacentHTML(
          "beforeend",
          (data.filterOptions?.steps || [])
            .map(
              (item) =>
                `<option value="${esc(item)}">Step ${esc(item)}</option>`,
            )
            .join(""),
        );
        stepSelect.dataset.loaded = "true";
      }
      const body =
  ...
  async function setup() {
    ...
    if (today)
      today.textContent = new Date().toLocaleString("en-GB", {
        day: "numeric",
        month: "short",
        year: "numeric",
        hour: "2-digit",
          ...
          users
            .map((user) => {
              const name =
                ...
                user.email;
              return `<option value="${esc(user._id)}">${esc(name)}</option>`;
            })
            .join(""),
        );
    } catch (_) {
      /* Main table still reports its own data error if access is unavailable. */
    }
    let timer;
    document
      .getElementById("adminTrackingSearch")
    ...
    document
      .getElementById("adminTrackingPageSize")
    ...
    document
      .getElementById("adminTrackingRefresh")
      ?.addEventListener("click", render);
    document
      .getElementById("adminTrackingPagination")
      ?.addEventListener("click", (event) => {
        const button = event.target.closest("[data-tracking-page]");
        if (!button || button.disabled) return;
        state.page = Number(button.dataset.trackingPage);
        render();
      });
    document
      .getElementById("adminTrackingPagination")
      ?.addEventListener("change", (event) => {
        if (event.target.id !== "adminTrackingBottomPageSize") return;
        state.limit = Number(event.target.value);
        document.getElementById("adminTrackingPageSize").value = String(
          state.limit,
        );
        state.page = 1;
        render();
    ...
    table.addEventListener("click", (event) => {
      ...
      if (!button) return;
      const item = state.rows[Number(button.dataset.trackingView)];
      if (!item) return;
      const detail = `Lead: ${item.lead_name || "—"}\nEmail: ${item.lead_email || "—"}\nSubject: ${item.subject || "—"}\nStatus: ${item.status_badge || "—"}\nOwner: ${item.full_name || "—"}`;
      window.Swal
        ? Swal.fire({ title: "Tracking details", text: detail, icon: "info" })
        : window.alert(detail);
    });
    const exportRows = () =>
      state.rows.map((item) => ({
        "Lead Name": item.lead_name,
        "Lead Email": item.lead_email,
    ...
    const download = (extension, separator) => {
      ...
      const content = [
        ...
        ...rows.map((row) =>
          headers.map((header) => encode(row[header])).join(separator),
        ),
      ].join("\n");
      ...
      link.download = `all-tracking-report.${extension}`;
      link.click();
      URL.revokeObjectURL(link.href);
    };
    document
      .getElementById("adminTrackingExportCsv")
      ?.addEventListener("click", () => download("csv", ","));
    document
      .getElementById("adminTrackingExportExcel")
