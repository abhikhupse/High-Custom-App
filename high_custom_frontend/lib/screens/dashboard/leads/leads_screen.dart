...
class _LeadsScreenState extends State<LeadsScreen> {
  ...
  final TextEditingController editCompanyController = TextEditingController();
  ...
  void _startTrackingRefresh() {
    ...
    debugPrint('✅ Leads automatic tracking refresh started.');
  ...
  Future<void> _refreshTrackingStatus() async {
    ...
    try {
      ...
      for (final item in serverLeads) {
        if (item is Map) {
          refreshed.add(_normalizeLead(Map<String, dynamic>.from(item)));
        }
      }
      ...
      debugPrint('Automatic tracking refresh error: $error');
    } finally {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        if (serverLeads is List) {
          for (final item in serverLeads) {
            if (item is Map) {
              loaded.add(_normalizeLead(Map<String, dynamic>.from(item)));
            }
          }
        }
        ...
        _showMessage(
          response['message']?.toString() ?? 'Unable to load leads.',
        );
      }
    } catch (error) {
      debugPrint('Load leads error: $error');

      if (mounted) {
        _showMessage('Unable to load leads.');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Map<String, dynamic> _normalizeLead(Map<String, dynamic> lead) {
    final addedDate = _parseDate(lead['addedDate'] ?? lead['createdAt']);

    final updatedDate = _parseDate(lead['updatedDate'] ?? lead['updatedAt']);

    final tracking = lead['tracking'] == true;

    String trackingStatus = lead['trackingStatus']?.toString().trim() ?? '';

    if (trackingStatus.isEmpty) {
      trackingStatus = tracking ? 'Pending' : 'Skip';
    }
  ...
  int get contactedCount {
    return _summaryLeads.where((lead) {
      final status = lead['trackingStatus']?.toString().toLowerCase() ?? '';
  ...
  int get openedCount {
    return _summaryLeads.where((lead) {
      ...
      return status == 'opened' || status == 'open' || status == 'seen';
    }).length;
  }
  ...
  int get repliedCount {
    return _summaryLeads.where((lead) {
      ...
      return status == 'replied' || status == 'reply';
    }).length;
  }
  ...
  List<Map<String, dynamic>> get filteredLeads {
    List<Map<String, dynamic>> result = List<Map<String, dynamic>>.from(leads);
    ...
    if (query.isNotEmpty) {
      result = result.where((lead) {
        final firstName = lead['firstName']?.toString().toLowerCase() ?? '';

        final lastName = lead['lastName']?.toString().toLowerCase() ?? '';
    ...
    if (selectedTypeFilter != 'All Types') {
      result = result.where((lead) {
        final type = lead['type']?.toString().toLowerCase() ?? '';

        return type == selectedTypeFilter.toLowerCase();
      }).toList();
    }
    ...
    if (selectedStatusFilter != 'All Status') {
      result = result.where((lead) {
        ...
        final wanted = selectedStatusFilter.toLowerCase();
  ...
  List<Map<String, dynamic>> get paginatedLeads {
    ...
    final calculatedEnd = start + entriesPerPage;

    final end = calculatedEnd > data.length ? data.length : calculatedEnd;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          onRefresh: () async {
            await _loadLeads(showLoader: false, resetPage: false);
          },
          child: LayoutBuilder(
              ...
              if (isLoading)
                ...
                ...paginatedLeads.map(
                  (lead) => Padding(
                    ...
                    child: _buildMobileLeadCard(lead),
                  ),
                ),

              if (!isLoading && filteredLeads.isNotEmpty) ...[
    ...
    return Row(
      ...
      children: [
        ...
        IconButton(
          ...
          icon: const Icon(Icons.search_rounded, color: lightText, size: 30),
        ),

        if (widget.canExport && widget.canImport) const SizedBox(width: 4),

        InkWell(
          onTap: _openAddLeadScreen,
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                ...
                colors: [Color(0xFFFFD66F), Color(0xFFF0B93C)],
              ),
              borderRadius: BorderRadius.circular(13),
              boxShadow: [
                BoxShadow(
  ...
  Widget _buildSummaryCards() {
    final contactedPercentage = _percentage(contactedCount, totalLeadsCount);

    final openedPercentage = _percentage(openedCount, contactedCount);

    final repliedPercentage = _percentage(repliedCount, contactedCount);
          ...
          children: [
            SizedBox(
              ...
              child: _summaryCard(
                ...
                subtitle: '$todayAddedCount added today',
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: const InputDecoration(
          ...
          prefixIcon: Icon(Icons.search_rounded, color: mutedText, size: 27),
          hintText: 'Search by name, email, company...',
          hintStyle: TextStyle(color: Color(0xFF74767E), fontSize: 12.5),
      ...
      builder: (context, constraints) {
          ...
          return Column(
            children: [
              Row(
                children: [
                  Expanded(
                    child: _mobileDropdownButton(
                      value: selectedTypeFilter,
                      items: const ['All Types', 'Email', 'WhatsApp'],
              ...
              SizedBox(width: double.infinity, child: _mobileFilterButton()),
            ],
          );
        }
        ...
        return Row(
          children: [
            ...
            Expanded(child: _mobileFilterButton()),
          ],
        );
      },
    );
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            const Icon(Icons.filter_alt_rounded, color: gold, size: 20),
  ...
  Widget _buildMobileExcelActions() {
    return Row(
      children: [
        if (widget.canImport)
          Expanded(
            child: OutlinedButton.icon(
              onPressed: isDownloadingExcel ? null : _downloadExcel,
              icon: const Icon(Icons.file_download_outlined, size: 18),
              label: Text(isDownloadingExcel ? 'Downloading...' : 'Export'),
        ...
        Expanded(
          child: OutlinedButton.icon(
            onPressed: isUploadingExcel ? null : _uploadExcel,
            icon: const Icon(Icons.file_upload_outlined, size: 18),
            label: Text(isUploadingExcel ? 'Uploading...' : 'Import'),
            style: OutlinedButton.styleFrom(
              foregroundColor: lightText,
              disabledForegroundColor: mutedText,
              side: const BorderSide(color: borderColor),
  ...
  Widget _buildMobileLeadCard(Map<String, dynamic> lead) {
    ...
    final company = lead['company']?.toString().trim() ?? '';

    final type = lead['type']?.toString() ?? 'Email';

    final addedDate = lead['addedDate'] as DateTime;

    final displayName = fullName.isEmpty ? email : fullName;

    final initials = _getInitials(firstName, lastName, email);
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          displayName.isEmpty ? 'Unnamed Lead' : displayName,
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            color: white,
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                _formatShortCardDate(addedDate),
                                maxLines: 3,
                                style: const TextStyle(
                                  color: mutedText,
                                  fontSize: 10.5,
  ...
  Widget _leadMenu(Map<String, dynamic> lead) {
    return PopupMenuButton<String>(
      padding: EdgeInsets.zero,
      constraints: const BoxConstraints(minWidth: 125),
      color: cardBackground2,
      icon: const Icon(Icons.more_vert_rounded, color: mutedText, size: 23),
      onSelected: (value) {
        if (value == 'view') {
          _showViewLeadDialog(lead);
        } else if (value == 'edit') {
      ...
      itemBuilder: (context) => const [
        PopupMenuItem(
          ...
          child: Row(
            children: [
              Icon(Icons.visibility_outlined, color: lightText, size: 18),
              SizedBox(width: 10),
              Text('View', style: TextStyle(color: lightText)),
            ],
          ),
        ),
        ...
        PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 10),
              Text('Edit', style: TextStyle(color: lightText)),
            ],
          ),
        ),
  ...
  Widget _trackingBadge(Map<String, dynamic> lead) {
    final rawStatus = lead['trackingStatus']?.toString().trim() ?? '';

    final status = rawStatus.isEmpty
        ? (lead['tracking'] == true ? 'Pending' : 'Skip')
        : rawStatus;
    ...
    if (normalized == 'interested') {
      ...
      icon = Icons.visibility_rounded;
    } else if (normalized == 'clicked' || normalized == 'click') {
      color = purple;

      icon = Icons.touch_app_rounded;
    } else if (normalized == 'replied' || normalized == 'reply') {
      color = purple;

      icon = Icons.reply_rounded;
    } else if (normalized == 'failed' || normalized == 'fail') {
      color = red;
  ...
  String _getInitials(String firstName, String lastName, String email) {
    ...
    if (result.isEmpty && email.isNotEmpty) {
  ...
  Color _avatarColor(String value) {
    final colors = [
      const Color(0xFF4D95FF),
      const Color(0xFF45CB7A),
      const Color(0xFF8D54FF),
      const Color(0xFFFFC63D),
      const Color(0xFFFF5666),
      const Color(0xFF38C3D8),
    ];
    ...
    final sum = value.codeUnits.fold<int>(0, (previous, element) {
  ...
  Widget _buildMobilePagination() {
    ...
    final start = data.isEmpty ? 0 : (currentPage - 1) * entriesPerPage + 1;

    final calculatedEnd = currentPage * entriesPerPage;
  ...
  Widget _paginationControls(int pages) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      mainAxisSize: MainAxisSize.min,
      children: [
        ...
        _pageNumber(currentPage, selected: true),

        if (currentPage < pages) ...[
          const SizedBox(width: 8),

          _pageNumber(currentPage + 1, selected: false),
        ],
        ...
        if (currentPage + 1 < pages) ...[
          ...
          _pageNumber(pages, selected: false),
        ],
  ...
  Widget _pageNumber(int page, {required bool selected}) {
    return InkWell(
      onTap: selected
          ? null
          : () {
      ...
      child: Container(
        ...
        alignment: Alignment.center,
        decoration: BoxDecoration(
          color: selected ? gold.withOpacity(0.10) : cardBackground,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: selected ? gold : borderColor),
        ),
        child: Text(
          '$page',
          style: TextStyle(
    ...
    return SizedBox(
      ...
      child: OutlinedButton(
        ...
        style: OutlinedButton.styleFrom(
          padding: EdgeInsets.zero,
          foregroundColor: lightText,
          disabledForegroundColor: mutedText.withOpacity(0.25),
          side: const BorderSide(color: borderColor),
  ...
  Widget _entriesDropdown({bool compact = false}) {
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<int>(
          ...
          style: TextStyle(color: lightText, fontSize: compact ? 10 : 12),
          items: const [
  ...
  Widget _buildMobileLoading() {
    return const AppCardSkeletonList(itemCount: 5, compact: true);
  }
  ...
  Widget _buildMobileEmpty() {
    return Container(
      ...
      child: const Column(
        children: [
          Icon(Icons.people_outline_rounded, color: gold, size: 44),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(flex: 3, child: _buildMobileSearch()),

              const SizedBox(width: 12),

              Expanded(flex: 2, child: _buildMobileFilters()),
            ],
          ),
          ...
          Align(
            ...
            child: SizedBox(width: 300, child: _buildMobileExcelActions()),
          ),
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                return Wrap(
                  spacing: 16,
                  runSpacing: 16,
                  children: paginatedLeads.map((lead) {
                    return SizedBox(
                      width: cardWidth,
                      child: _buildMobileLeadCard(lead),
                    );
  ...
  Future<void> _openAddLeadScreen() async {
    ...
    await _loadLeads(showLoader: false);
  }
  ...
  void _showViewLeadDialog(Map<String, dynamic> lead) {
    final firstName = lead['firstName']?.toString().trim() ?? '';
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.person_outline, color: gold),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                fullName.isEmpty ? 'Lead Details' : fullName,
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: const TextStyle(
                                  color: white,
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _viewInfoRow('Type', type, Icons.category_outlined),
  ...
  Widget _viewInfoRow(String label, String value, IconData icon) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  void _showEditLeadDialog(Map<String, dynamic> lead) {
    editEmailController.text = lead['email']?.toString() ?? '';

    editFirstNameController.text = lead['firstName']?.toString() ?? '';

    editLastNameController.text = lead['lastName']?.toString() ?? '';

    editCompanyController.text = lead['company']?.toString() ?? '';

    selectedType = lead['type']?.toString() ?? 'Email';

    trackingEnabled = lead['tracking'] == true;
                            ...
                            Expanded(
                              ...
                              child: ElevatedButton.icon(
                                onPressed: isUpdatingLead
                                    ? null
                                    : () {
                                        _updateLead(dialogContext, lead);
                                      },
                                icon: isUpdatingLead
                                    ? const SizedBox(
                                        width: 15,
  ...
  Future<void> _updateLead(
    ...
    if (leadId.isEmpty) {
      _showMessage('Lead ID not found.');
    ...
    final company = editCompanyController.text.trim();
    ...
    try {
      final response = await LeadsApi.updateLead(
        leadId: leadId,
        email: email,
        firstName: firstName,
        lastName: lastName,
      ...
      if (response['success'] == true) {
        ...
        _showMessage(
          response['message']?.toString() ?? 'Lead updated successfully.',
        );

        await _loadLeads(showLoader: false, resetPage: false);
      } else {
        _showMessage(
          response['message']?.toString() ?? 'Unable to update lead.',
        );
      }
    } catch (error) {
      debugPrint('Update lead error: $error');

      if (mounted) {
        _showMessage('Unable to update lead.');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  void _deleteLead(Map<String, dynamic> lead) {
    final firstName = lead['firstName']?.toString() ?? '';

    final lastName = lead['lastName']?.toString() ?? '';
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton.icon(
              onPressed: isDeletingLead
                  ? null
                  : () async {
                      Navigator.pop(dialogContext);

                      await _performDeleteLead(lead);
                    },
              icon: const Icon(Icons.delete_outline, size: 17),
              label: const Text('Delete'),
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFFC94141),
  ...
  Future<void> _performDeleteLead(Map<String, dynamic> lead) async {
    final leadId = lead['_id']?.toString() ?? '';
    ...
    try {
      final response = await LeadsApi.deleteLead(leadId: leadId);
      ...
      if (response['success'] == true) {
        _showMessage(
          response['message']?.toString() ?? 'Lead deleted successfully.',
        );

        await _loadLeads(showLoader: false, resetPage: false);
      } else {
        _showMessage(
          response['message']?.toString() ?? 'Unable to delete lead.',
        );
      }
    } catch (error) {
      debugPrint('Delete lead error: $error');

      if (mounted) {
        _showMessage('Unable to delete lead.');
      }
    } finally {
      if (mounted) {
        setState(() {
                      ...
                      child: Column(
                        children: [
                          _dateSelector(
                            ...
                            value: tempFrom,
                            onTap: () async {
                              final picked = await _pickDate(tempFrom);
                          ...
                          _dateSelector(
                            ...
                            value: tempTo,
                            onTap: () async {
                              final picked = await _pickDate(tempTo);
  ...
  Future<DateTime?> _pickDate(DateTime? value) {
  ...
  Future<void> _uploadExcel() async {
    try {
      ...
      if (bytes.isEmpty) {
        _showMessage('Unable to read selected Excel file.');
      ...
      final response = await LeadsApi.uploadLeadsExcel(
        bytes: bytes,
        fileName: file.name,
      );
      ...
      if (response['success'] == true) {
        ...
        _showMessage(
          response['message']?.toString() ?? 'Unable to import Excel file.',
        );
      }
    } catch (error) {
      debugPrint('Excel upload error: $error');

      if (mounted) {
        _showMessage('Excel upload failed.');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _downloadExcel() async {
    if (leads.isEmpty) {
      _showMessage('There are no leads to download.');
    ...
    try {
      ...
      for (final lead in leads) {
        rows.add([
          ...
          _formatDate(lead['addedDate'] as DateTime),
        ]);
      }
      ...
      for (int i = 0; i < rows.length; i++) {
        ...
        for (final value in rows[i]) {
          ...
          if (i == 0) {
            html.write('<th>$escaped</th>');
          } else {
            html.write('<td>$escaped</td>');
          }
        }
      ...
      if (mounted) {
        _showMessage('Excel file prepared successfully.');
      }
    } catch (error) {
      debugPrint('Excel download error: $error');

      if (mounted) {
        _showMessage('Unable to download Excel file.');
      }
    } finally {
      if (mounted) {
        setState(() {
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText: hint,
            hintStyle: const TextStyle(color: Color(0xFF687083), fontSize: 12),
            prefixIcon: Icon(icon, color: mutedText, size: 18),
            filled: true,
            fillColor: cardBackground2,
  ...
  String _formatShortCardDate(DateTime date) {
    const months = [
      'Jan',
      'Feb',
      'Mar',
